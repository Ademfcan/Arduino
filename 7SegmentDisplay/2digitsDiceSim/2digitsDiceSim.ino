...
void loop() {
  ...
  if (isOn) {
    for (int i = 0; i < 25; i++) {
      // cycle between random values to give that dice effect
      // cycle slower and slower to make it realistic
      ...
      if (i < 18) {
        ...
        if (i == 24) {
          // reuse lastdigit to store the final random number
          lastDigit = rand;
          counterDelay = 200;
    ...
    // default will be zero, or last number selected
    int right = lastDigit % 10;
    int left = lastDigit / 10;
    // set each left/right sides on/off very quickly to make it appear as both sides are on at the same time
    // this is the only way due to the display having shared pins
    bool isLeft = true;
    for (int j = 0; j < counterDelay; j += switchDelay) {
